...
### 17:34 - Replace placeholder card art links with hotlinkable image URLs [openai/gpt-5-codex]

**User Prompt:**
can you find fitting image urls that can be hotlinked? If so insert them in the card markdowns

**Issues Encountered:**
- Wikimedia Commons began rate-limiting repeated file checks during URL validation, so not every candidate URL could be re-verified in the same run.

**Decisions Made:**
- Replaced all placeholder `cdn.fab.example` image URLs in themed card markdown files with hotlinkable Wikimedia Commons `Special:FilePath` URLs.
- Chose image subjects aligned to each card concept/theme (marine, urban nocturne, and sky/orbital motifs).
- Reused one station photo for two municipal cards where it best matched platform-related flavor.

**Technical Info Consulted:**
- Wikimedia Commons API (`action=query&list=search` with file namespace)
- `cards/Glass-Tide-Consortium.md`
- `cards/Municipal-Nocturne.md`
- `cards/Skylab-Bestiary.md`
- `AGENTS.md`
...
**Assumptions Made:**
- Wikimedia Commons `Special:FilePath` endpoints are acceptable hotlink targets for in-game card art references.

**Other Notes:**
- URLs remain parser-compatible with the existing markdown schema.

### 17:37 - Render card art URLs in CardComponent [openai/gpt-5-codex]

**User Prompt:**
display the images in the card component

**Issues Encountered:**
- None.

**Decisions Made:**
- Replaced the static art placeholder region with conditional rendering based on `card.image`.
- Added a styled `<img>` element with `object-fit: cover` to present remote card art cleanly within the existing card frame.
- Kept the placeholder text fallback when `card.image` is not available.
...
**Assumptions Made:**
- Card image URLs are externally hosted and directly embeddable in browser `<img>` tags.

**Other Notes:**
- Image loading uses `loading=\"lazy\"` to reduce initial UI load cost.
